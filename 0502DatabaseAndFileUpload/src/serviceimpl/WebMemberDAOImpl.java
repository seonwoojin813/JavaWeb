...
public String idcheck(String id) {
	String result = null;
	try { pstmt = con.prepareStatement("select id from webmember where upper(trim(id))=?");
    	...
    	System.out.println("id 중복 검사:" + e.getMessage());
    	e.printStackTrace();
	}finally {
		try {
	        pstmt.close();		
...
public int insertuser(WebMember webMember) {
	...
	try{
		pstmt = 
			con.prepareStatement(
					"insert into webmember(id, pw, nickname) "
					+ "values(?,?,?)");

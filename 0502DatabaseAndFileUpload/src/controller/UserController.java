...
public class UserController extends HttpServlet {
   ...
   private WebMemberService webMemberService;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//동일한 요청 URL 전송 방식을 이용해서 구분하고자 하는 경우
	 	//request.getMethod()를 이용합니다.
		...
		case "user/register":
			...
			if(request.getMethod().equals("GET")) {
				//단순 페이지이동은 포워딩
				//디렉톻리 패턴을 사용할 때는 결과 페이지에 포함되기 때문에 
				//디렉토리를 제거하고자 하면 ../로 시작
		...
		break;
		case "user/idcheck":
			JSONObject jsonObject = webMemberService.idcheck(request);
		...
		case "user/nicknamecheck":
			JSONObject jsonObject1 = webMemberService.nicknamecheck(request);
			//메소드 수행 결과를 request에 저장
		...
		case "user/login":
			...
			dispatcher.forward(request, response);
				...
				WebMember webMember = webMemberService.login(request);
				if(webMember == null) {

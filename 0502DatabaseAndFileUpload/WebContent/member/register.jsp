...
<body>
<!-- 최근에는 form에 action을 기재하지 않고 method를 post로 설정해서 get 요청과 구분해서 처리합니다. -->
...
<script>
   ...
   //아이디 입력란에서 포커스가 벗어날 때 ajax로 중복체크  
   ids.addEventListener("focusout", function(e){
	   //아래메시지가 출력되지 않으면 이벤트 에러이고
	   //아래 메시지는 뜨는데 Controller에서 출력이 안되면 URL을 잘못 설정
	  console.log('메시지');
	   ...
	   $.ajax({
		  ...
		  success:function(data){
			  if(data.result === "success"){
				  ...
				  idmsg.innerHTML = "사용 불가능한 아이디입니다."
   ...
   nicknameconfirm.addEventListener('click', function(e){
     ...
     //form의 데이터를 전송하고자 할 때
   registerform.addEventListener("submit", function(e){
	   //id 중복검사 결과 확인
	   if(idcheck == false){
		idmsg.innerHTML = "id 중복검사를 수행해야 합니다."
		//폼의 데이터 전송을 하지 않도록 설정
		e.preventDefault()
	}
	   //nickname 중복검사 결과 확인
	   if(nicknamecheck == false){
		nicknamemsg.innerHTML = "nickname 중복검사를 수행해야 합니다."
		//폼의 데이터 전송을 하지 않도록 설정
		e.preventDefault()
	} 

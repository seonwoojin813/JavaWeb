...
public class ItemDAOImpl implements ItemDAO {
	...
	private ItemDAOImpl() {
		...
		//인스턴스에 자신의 인스턴스 변수의 데이터를 직접 생하지 않고 외부에서 생성한 데이터를 대입하는 것	
		//**중요 : 만드는 것에 신경쓰지 않고 사용하는 부분에만 집중 
		//비지니스 로직만 생성하면 됨
		con = GlobalApplication.shardInstance().con;		
	}
	
	//클라이언트나 다른 개발자들은 구현될 코드를 볼 필요가 없기 때문에 메소드 모양만 보면 되므로 변수는 인터페이스 타입으로 선언
	...
	//1개를 리턴해야 하는 경우는 리턴할 데이터를 null로 초기화해서 리턴
	@Override
	public Integer codeCheck(int code) {
	  ...
	  // PreparedStatement pstmt = null;
	   ResultSet rs = null;
	   try {
		   //매개변수로 받은 데이터의 존재여부를 확인하기 위한 sql 실행
	...
	public int insertItem(Item item) {
		...
		try {
			...
			/*
			pstmt = con.prepareStatement("insert into item(code, title, category, description) "
					+ "values(?,?,?,?)");
					*/
			CallableStatement call = con.prepareCall("{call insertitem(?,?,?,?)}");
		    ...
		    call.setInt(1, item.getCode());
			call.setString(2, item.getTitle());
		    call.setString(3, item.getCategory());
		    call.setString(4, item.getDescription());
	...
	//상세보기 처럼 하나의 데이터를 찾아오는 메소드는 리턴할 데이터를 null로 초기화하고 리턴
	@Override
	public Item detailItem(int code) {
		...
		ResultSet rs = null;
		try {
			pstmt = con.prepareStatement("select code, title, category, description "
	...
	public int updateItem(Item item) {
		...
		try { 
			...
			pstmt = con.prepareStatement("update item set title=?, category=?, description=? "
					+ "where code=?");

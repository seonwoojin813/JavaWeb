...
<body>
    ...
    <!-- 채팅 작성자를 분하기 위한 입력 상자 - 로그인이 있으면 필요없음 -->
    ...
    <!-- 전송받은 메시지를 출력할 영역 -->
   받은메시지<textarea id="disp" rows="20" cols="50"></textarea>
   
	<p><a href="pwsend"><img src="img/but.png" width="150px" height="50px" />
	</a></p>
	...
	<script>
		//웹 소켓 서버에 접속
		var webSocket = new WebSocket("ws://192.168.0.108:8080/etc_(1)/websocket")
		...
		webSocket.addEventListener("open", function(e){
			...
			document.getElementById("sendbtn").addEventListener(
					"click", function (e){
					//nickname과 msg에 입력된 내용을 웹 소켓 서버에 전송
					webSocket.send(
		...
		webSocket.addEventListener("message", function(e){
			//alert(e.data)
			//새로 전송되어온 메시지를 출력하고 그 아래에 기본 메시지를 출력
	...
	<script>
		var ar = ["captin.jpg", 'ironman.jpg', 'Thor.jpg']
		document.getElementById("pushbtn").addEventListener(
			...
			eventSource.addEventListener('message', function(e1){
				document.getElementById('pushdisp').src = 
					ar[parseInt(e1.data)]
			});
